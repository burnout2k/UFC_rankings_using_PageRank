...
def parse_fight_details(soup):
    ...
    for tag in soup.find_all('li', class_='b-list__box-list-item'):
        match = re.search(r'Date:\s*</i>\s*(.*)\s*</li>', str(tag))
        if match:
            date = match.group(1).strip()
            fight_details_df['DATE'] = date
...
def organise_fight_results(results_from_soup, fight_results_column_names):
    ...
    fight_results_clean = []
    # append event name
    fight_results_clean.append(results_from_soup[0].strip())
    fight_results_clean.append(results_from_soup[1])
    fight_results_clean.append(results_from_soup[2])
    fight_results_clean.append(results_from_soup[3])
    fight_results_clean.append(results_from_soup[4])
    fight_results_clean.append(results_from_soup[5])
    fight_results_clean.append(results_from_soup[6].replace("Method: ", ""))
    fight_results_clean.append(results_from_soup[7].replace("Round:", ""))
    fight_results_clean.append(results_from_soup[8].replace("Time:", ""))
    fight_results_clean.append(results_from_soup[10].replace("Referee:", ""))
    #del fight_results_clean[0]
    #print(fight_results_clean)
...
def parse_fighter_tott(soup):
    ...
    tott = soup.find_all('ul', class_='b-list__box-list')[0]
    tott2 = soup.find_all('ul', class_='b-list__box-list')[1]
    tott3 = soup.find_all('ul', class_='b-list__box-list')[2]
    ...
    for tag in tott2.find_all('i'):
        # add text together and append to fighter_tott
        fighter_tott.append(tag.text + tag.next_sibling)
    for tag in tott3.find_all('i'):
        # add text together and append to fighter_tott
        fighter_tott.append(tag.text + tag.next_sibling)
    # clean each element in the list, removing '\n' and '  '
